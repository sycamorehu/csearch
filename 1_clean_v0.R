...
#*-- **hotelnum ------------------------------------------------------------------
names(dt)
hotelnum2 <- hotelnum[, .(hotelnum = round(mean(hotelnum),0)), by = qid_new]


sc2 <- sc[hotelnum2, on = "qid_new"]
...
sc[dist_qry == "不限", dist := ""]
sc[dist_qry == "4", dist := ""]
sc[dist_qry == "1", dist := 1000]
sc[dist_qry == "2", dist := 2000]
sc[dist_qry == "3", dist := 3000]
sc[dist_qry == "5", dist := 5000]
sc[dist_qry == "6", dist := 6000]

sc[dist_qry %like% "公里内", dist := 1000*as.numeric(str_match(dist_qry,
                                                            "([0-9]+.[0-9]+)")[, 2])]
sc[dist_qry %like% "公里内" & is.na(dist), 
   dist := 1000*as.numeric(str_match(dist_qry,"([0-9]+)")[, 2])]
sc[dist == 4000, dist := ""]
